...
export const Route = createFileRoute("/agenda")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Reserve seu horário online em poucos passos. Confirmação imediata." },
    ],
  }),
...
const PROFESSIONALS = [
  { id: "equipe", name: "Equipe MR Odontologia", text: "A equipe direciona seu atendimento para o profissional ideal conforme o tratamento escolhido." },
];
...
function AgendaPage() {
  ...
  const { data: busy, isFetching: busyLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      return (data ?? []).map((r: any) => new Date(r.scheduled_at).toTimeString().slice(0, 5));
    },
  });
  ...
  const canNext =
    ...
    (step === 2 && !!date) ||
    (step === 3 && !!time) ||
    (step === 4 && form.name.trim().length > 1 && /\S+@\S+\.\S+/.test(form.email) && form.phone.trim().length >= 8);
  ...
  async function submit() {
    ...
    dt.setHours(h, m, 0, 0);
    const notes = [form.notes.trim(), `Profissional/triagem: ${selectedProfessional.name}`].filter(Boolean).join("\n");
    const { data, error } = await supabase
      .from("appointments")
      .insert({
      ...
      <div className="mx-auto max-w-[1120px]">
        <div className="mb-10 grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <div>
            ...
            <h1 className="mt-5 text-5xl text-[var(--petrol)] md:text-6xl">Agende seu horário sem complicação.</h1>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="flex items-center gap-3 rounded-2xl bg-[var(--ivory)] p-3">
                  <span className="grid h-8 w-8 place-items-center rounded-full bg-[var(--petrol)] text-sm font-semibold text-white">{number}</span>
        ...
        <div className="overflow-hidden rounded-[1.75rem] border border-white bg-white shadow-[var(--shadow-luxe)]">
          <div className="grid border-b border-border bg-[var(--ivory)] sm:grid-cols-3 lg:grid-cols-6">
            {STEPS.map((label, i) => (
              <div key={label} className={`flex items-center gap-3 border-b-2 px-4 py-4 transition-colors ${i === step ? "border-[var(--gold)] bg-white" : "border-transparent"}`}>
                ...
                <span className={`text-xs font-semibold uppercase tracking-[0.08em] ${i === step ? "text-[var(--petrol)]" : "text-muted-foreground"}`}>{label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="p-6 md:p-10">
            {step === 0 && (
              <Panel title="Escolha o tratamento" text="Comece selecionando o serviço desejado. Você pode ajustar depois com a equipe.">
                {servicesLoading ? (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {services?.map((s) => {
                      ...
                      return (
                        <button key={s.id} onClick={() => setServiceSlug(s.slug)} className={`rounded-2xl border p-5 text-left transition-all duration-300 ${selected ? "border-[var(--petrol)] bg-[var(--petrol)] text-white shadow-[var(--shadow-soft)]" : "border-border bg-[var(--ivory)] hover:-translate-y-0.5 hover:border-[var(--gold)] hover:shadow-[var(--shadow-soft)]"}`}>
              ...
              <Panel title="Profissional responsável" text="A clínica organiza internamente o profissional mais adequado para o procedimento.">
                <div className="grid gap-4">
                  {PROFESSIONALS.map((item) => {
                    ...
                    return (
                      <button key={item.id} onClick={() => setProfessional(item.id)} className={`rounded-2xl border p-6 text-left transition-all ${selected ? "border-[var(--petrol)] bg-[var(--clinical-light)]" : "border-border bg-white hover:-translate-y-0.5 hover:border-[var(--gold)]"}`}>
            ...
            {step === 2 && (
              <Panel title="Escolha a data" text={`Dias disponíveis para ${selectedService?.name ?? "o atendimento"}.`}>
                <Calendar month={month} setMonth={setMonth} value={date} onChange={(d) => { setDate(d); setTime(null); }} />
              </Panel>
            )}
...
function Calendar({ month, setMonth, value, onChange }: {
  month: Date;
  ...
  return (
    <div className="rounded-[1.5rem] border border-border bg-[var(--ivory)] p-5">
      <div className="mb-4 flex items-center justify-between">
        <button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} className="grid h-10 w-10 place-items-center rounded-full border border-border bg-white text-lg transition-colors hover:bg-primary hover:text-primary-foreground">‹</button>
        <div className="font-serif text-xl capitalize text-[var(--petrol)]">{month.toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</div>
        <button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} className="grid h-10 w-10 place-items-center rounded-full border border-border bg-white text-lg transition-colors hover:bg-primary hover:text-primary-foreground">›</button>
      </div>
      <div className="grid grid-cols-7 gap-2 text-center">
        {WEEKDAYS.map((w) => <div key={w} className="py-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{w}</div>)}
        {cells.map((d, i) => {
